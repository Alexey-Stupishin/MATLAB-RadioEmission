...
try
    ...
    if ~isempty(out) && (isfield(out, 'BZ') || isfield(out, 'BOX')) % SAV
        if isfield(out, 'BOX')
            mfoData.filetype = 'BOX';
%             mfoData.B.x = permute(out.BOX.BY, [2 1 3]);
%             mfoData.B.y = permute(out.BOX.BX, [2 1 3]);
%             mfoData.B.z = permute(out.BOX.BZ, [2 1 3]);
            mfoData.B.x = out.BOX.BX;
            mfoData.B.y = out.BOX.BY;
            mfoData.B.z = out.BOX.BZ;
            if isfield(out.BOX, 'MODSTEP')
                mfoData.stepP = out.BOX.MODSTEP * ones(1, 3);
                sz = size(mfoData.B.z);
                mfoData.baseP = - mfoData.stepP(1:2) .* sz(1:2)/2;
            end
            if isfield(out.BOX, 'VCOS')
                mfoData.vcos = out.BOX.VCOS;
            end
            if isfield(out.BOX, 'RSUN')
                mfoData.R = out.BOX.RSUN;
            end
        else
            ...
            if ~isfield(out, 'SST_VERSION')
                ...
                mfoData = l_mfosavnew(mfoData, out);
            end
        end
    else
...
%--------------------------------------------------------------------------
function mfoData = l_mfosavcommon(mfoData, out, transp_need)
...
mfoData.magn = getfieldsafe(out, 'MAGN_BZ', []);
mfoData.magnetogram_src = getfieldsafe(out, 'MAGN_SRC', []);
mfoData.continuum_src = getfieldsafe(out, 'CONT_SRC', []);
if isfield(out, 'TIME')
...
mfoData = l_mfosavcommon(mfoData, out, false);

end

%--------------------------------------------------------------------------
function mfoData = l_mfosavnew(mfoData, out)

if strcmp(out.SST_VERSION, '20200228')
    transp_need = false;
elseif strcmp(out.SST_VERSION, '20210407')
    transp_need = true;
else
...
mfoData.version = out.SST_VERSION;
...
mfoData = l_mfosavcommon(mfoData, out, transp_need);
