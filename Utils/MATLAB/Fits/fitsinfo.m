...
%   $Revision: 1.1.6.17 $  $Date: 2011/05/17 02:24:47 $
...
if (~isStandardFile(info))
    ...
    warning(message('MATLAB:imagesci:fitsinfo:nonstandardFile', msg1, msg2));
...
if (atEOF(fid,fileSize) && ~endFound)
    ...
    warning(message('MATLAB:imagesci:fitsinfo:earlyEOF'));
...
if status == -1
  warning(message('MATLAB:imagesci:fitsinfo:headerUnitSeekFailed'));
end
%End READHEADERUNIT
...
card = card';
if readcount ~= 80
  error(message('MATLAB:imagesci:fitsinfo:cardbytes'));
end
...
switch lower(keyword)
 ...
 otherwise
  %NAXISN keywords
  if strfind(lower(keyword),'naxis')
    dim = sscanf(keyword(6:end),'%f');
    info(1).Size(dim) = value;
  end    
end    
...
if (datasize ~= 0)
    ...
    if (status == -1)
        ...
        if (id == OFFSETBEYONDEOF)
            ...
            warning(message('MATLAB:imagesci:fitsinfo:offsetBeyondEOF'));
      
        else
        
            error(message('MATLAB:imagesci:fitsinfo:seekFailed', msg));
...
if (fid == -1)
    ...
    error(message('MATLAB:imagesci:fitsinfo:fileOpen', filename));
...
while ~atEOF(fid,info.FileSize)
    ...
    if (~strcmpi(readCardImage(fid), 'xtension'))
        ...
        if status == -1
            ...
            warning(message('MATLAB:imagesci:fitsinfo:cardSeekFailed'));
...
otherwise
    ...
    error(message('MATLAB:imagesci:fitsinfo:unrecognizedExtension', extensionKeyword))
